...
void	FragTrap::rangedAttack(std::string const & target) {
	std::cout << BLUE << "[FR4G-TP <" << _name << ">]: " << YELLOW
...
void	FragTrap::takeDamage(unsigned int amount) {
	...
	std::cout << BLUE << "[FR4G-TP <" << _name << ">]: " << RED
	<< "(Take damage: -" << amount << ", now have:" << _hit_p << ")" << CYAN
	<< " Hit me, baby!" << std::endl << N;
	if (_hit_p == 0)
		std::cout << BLUE << "[FR4G-TP <" << _name << ">]: " << CYAN << "I can't feel my fingers! Gah! I don't have any fingers!" << std::endl << N;
}

void	 FragTrap::beRepaired(unsigned int amount) {
	_hit_p + amount > _max_hit_p ? _hit_p = _max_hit_p : _hit_p += amount;

	std::cout << BLUE << "[FR4G-TP <" << _name << ">]: " << GREEN
